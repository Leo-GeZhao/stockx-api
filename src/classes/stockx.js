...
module.exports = class Stockx {
  ...
  constructor(options = {}) {
    ...
    this.proxy =
      proxy == undefined || proxy.trim() == "" ? undefined : formatProxy(proxy);
  }
  ...
  async placeBid(product, options = {}) {
    ...
    if (!this.loggedIn)
      throw new Error("You must be logged in before placing a bid!");
    else if (amount == NaN)
      throw new Error(
        "Amount is incorrect, please ensure your parameters are correctly formatted."
      );
    else if (requestedSize == undefined)
      throw new Error("Please specify a size to bid on!");
    else if (product == undefined)
      throw new Error("A product must be specified!");
    else if (typeof product == "string")
      throw new Error(
        "The product passed in must an object. Use fetchProductDetails() to get the details first."
      );
    else if (product.variants == undefined)
      throw new Error(
        "No variants found in product! Please check the product object passed in."
      );

    //Get size from requestedSize in the product variants
    const size =
      requestedSize.toLowerCase() == "random"
        ? product.variants[Math.floor(Math.random() * product.variants.length)]
        : product.variants.find((variant) => variant.size == requestedSize);

    //Check if getting size was successful
    if (size == undefined)
      throw new Error("No variant found for the requested size!");
    if (size.uuid == undefined || size.uuid == "")
      throw new Error("No variant ID found for the requested size!");
  ...
  async placeAsk(product, options = {}) {
    ...
    //Verify fields passed in by user
    if (!this.loggedIn)
      throw new Error("You must be logged in before placing an ask!");
    else if (amount == NaN)
      throw new Error(
        "Amount is incorrect, please ensure your parameters are correctly formatted."
      );
    else if (requestedSize == undefined)
      throw new Error("Please specify a size to place an ask on!");
    else if (product == undefined)
      throw new Error("A product must be specified!");
    else if (typeof product == "string")
      throw new Error(
  ...
  async updateAsk(ask, options = {}) {
    ...
    if (!this.loggedIn)
      ...
      throw new Error(
        "Ask is incorrect, please ensure your parameters are correctly formatted."
      );
  ...
  async deleteAsk(ask) {
    //Verify fields passed in by user
    if (!this.loggedIn)
      throw new Error("You must be logged in before deleting an ask!");
    else if (ask == undefined)
      throw new Error(
        "Ask is incorrect, please ensure your parameters are correctly formatted."
      );
  ...
  async deleteBid(bid) {
    //Verify fields passed in by user
    if (!this.loggedIn)
      throw new Error("You must be logged in before deleting a bid!");
    else if (bid == undefined)
      throw new Error(
        "Bid is incorrect, please ensure your parameters are correctly formatted."
      );
